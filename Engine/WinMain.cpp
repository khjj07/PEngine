    ...
    _In_ int       nCmdShow)
{
    ...
    engine->hAccelTable= LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_ENGINE));
    engine->Run(scene);

    return (int) engine->msg.wParam;
}
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
	{
		...
		Engine::Instance()->render->Update(&hdc, Engine::Instance()->currentScene);
		EndPaint(hWnd, &ps);
	}
	break;
	case WM_DESTROY:
